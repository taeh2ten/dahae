		...
		<div id="wrapper">
			...
			<div id="main">
				...
				<table class="table table-stroped table-bordered table-hover" 
					...
					<thead>
					<tr>
						<th style="width:10%; text-align:center;"> ${str_num}</th>               <%-- 글번호 --%> 
						<th style="width:20%; text-align:center;"> ${str_goods_name}</th>        <%-- 상품명 --%> 
						<th style="width:10%; text-align:center;"> ${str_category_name}</th>     <%-- 카테고리명 --%>
						<th style="width:15%; text-align:center;"> ${str_image}</th>             <%-- 상품이미지 --%>
						<th style="width:10%; text-align:center;"> ${str_price}</th>             <%-- 판매액 --%>
						<th style="width:10%; text-align:center;"> ${str_reg_date}</th>          <%-- 등록일 --%>
					...
					<c:if test="${count != 0}">
						...
						<c:forEach var="goodsDto" items="${somegoods}" varStatus="status">	
							<tr onclick="location='goodsContent.do?pageNum=${pageNum}&num=${goodsDto.goods_number}'" style="cursor:pointer;">
								<td align="center">
									${number}
