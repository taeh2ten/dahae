...
/* 관리자 확인 */
function notice_admincheckRe(memId) {
	if(!memId ) {
...
/* 로그인 상태 확인 */
function notice_membercheckRe(memId) {
	if(! memId ) {
		alert( notmembererror );
		return false;		
	}
...
function modicheck(){
	if(! modiform.subject.value){
		...
		return false;
	}else if(! modiform.passwd.value){
		alert(passwderror);
		modiform.passwd.focus();
		return false;
	}
}

function modifycheck(){
	if(! modifyform.modify.value){
		alter(modifyerror);
		modifyform.modify.focus();
		return false;
	}
}
