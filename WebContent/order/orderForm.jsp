				...
				<div align="center">
					...
					<form method = "post" action = "orderPro.do"
						...
						<!-- 일단 상품 상세 페이지에서 상품 하나만 주문할 때 -->
							<tr>
								<th width="10%" style="text-align:center;"> ${str_goods_name} </th>
								<th width="8%" style="text-align:center;"> ${str_goods_img} </th>
								<th width="13%" style="text-align:center;"> ${str_goods_info} </th>
								<th width="3%" style="text-align:center;"> ${str_goods_count} </th>
								<th width="5%" style="text-align:center;"> ${str_goods_price} </th>
							</tr>
							<tr>
								...
								<c:if test = "${order == 1}">
									<c:forEach var="goodsDto" items="${goodsDto}">
										<tr>
											<td style="text-align:center;font-size:20px;"> ${goodsDto.goods_name} </td>
											<td style="text-align:center;">
												<img src = "${upload}${goodsDto.goods_images}" width = "180px" height = "auto">
											</td>
											<td style="text-align:center;">
												<pre style="text-align:center; font-size:18px; background:transparent; border:0;">${goodsDto.goods_info}</pre>
											</td>
											<c:set var = "cartDto" value = "<%=CartDBBean.getInstance()%>"/>
											<td style="text-align:center;font-size:20px;"> ${cartDto.getGoodsCount(sessionScope.memId, goodsDto.goods_number)}</td>
											<td style="text-align:center;font-size:20px;"> <fmt:formatNumber value="${goodsDto.price}"/> </td>
										</tr>
								...
								<c:if test = "${order == 0}">
									<tr>
										...
										<td> <pre style="text-align:center;font-size:20px;background:transparent;border:0;">${goodsDto.goods_info}</pre> </td>
										<td style="text-align:center;font-size:20px;"> ${goodsCount} </td>
										<td style="text-align:center;font-size:20px;"> <fmt:formatNumber value="${goodsDto.price}"/> </td>
									</tr>
								</c:if>
							<tr>		
							...
							<tr>
								...
								<td colspan = "4">
									<input class = "form-control" type = "text" name = "delivery_name" value = "${memberDto.name}" readonly>
								</td>
							</tr>
							<tr>
								...
								<c:if test = "${memberDto.tel == null}">
									<td colspan = "4">
										<input class = "form-control" type = "text" name = "delivery_tel1" 
											onkeyup="nexttel2()" maxlength="3" style="width:60px;"> -
										<input class = "form-control" type = "text" name = "delivery_tel2" 
											onkeyup="nexttel3()" maxlength="4" style="width:60px;"> -
							...
							<tr>
								...
								<c:if test = "${order == 1}"> 
									...
									<td colspan = "5"> <label> 합계 : </label> ${total} + 배송비 (+2500) = <label style="font-color:red;"> <fmt:formatNumber value="${total + 2500}"/> </label> </td>
								</c:if>
								<c:if test = "${order == 0}">
									<tr>
										<td colspan = "5"> <label> 합계 : </label> ${goodsDto.price * goodsCount} + 배송비 (+2500) = <label style="font-size:20px; color:red;"> <fmt:formatNumber value="${goodsDto.price * goodsCount + 2500}"/> </label> </td>
									</tr>
								</c:if>
							<tr>
